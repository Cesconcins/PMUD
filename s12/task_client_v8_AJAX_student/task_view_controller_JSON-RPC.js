...
function TaskVC(ajaxUrl, name = "Task", id = "#tasks") {
  this.name = name;
  this.id = id;
  this.client = new $.JsonRpcClient({ajaxUrl});
  ...
  TaskVC.prototype.listController = function() {
    ...
    let p1 = this.client.call('getAll', [where, this.order, (this.currentPage-1)*this.itemsOnPage, this.itemsOnPage]);
    let p2 = this.client.call('count', [where]);
    Promise.all([p1, p2])
    .then(([r1, r2]) => {
      let tasks = r1.result;
      let count = r2.result[0];
  ...
  TaskVC.prototype.createController = function() {
    ...
    this.client.call('add', [title, done])
    .then(r => {this.listController();}); 
  };
...
let task_vc = new TaskVC('http://localhost:9000');
let task_vch = new TaskVC('http://ubiwan.epsevg.upc.edu:3000', 'Remote task', '#home_tasks');
});
